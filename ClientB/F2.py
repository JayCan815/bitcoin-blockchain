...
turn = 2  # init to 1 per instructions - used to mine odd blocks
...
def mine():
    global balance
    clientPort = 20001 # client to send confirmed Tx to, change depending on which full node this is // CHANGE DEPENDING ON IF THIS IS F1 OR F2
    fullNodePort= 10000 # F2 full node, used to send the blocks // CHANGE DEPENDING ON IF THIS IS F1 OR F2
...
def main():
    global turn
    while 0 != 1:
        serverPort = 20000 #server port // CHANGE DEPENDING ON IF THIS IS F1 OR F2
        ...
        if clientAddress[0] == '127.0.0.1':
            ...
            if getsizeof(message) <= 59:
                ...
                bchain.close();
                print("Removing 4 Tx from Temp_T.txt")
                removeTx()
                clientPort = 20001
                clientSocket = socket(AF_INET, SOCK_DGRAM)
            ...
            if message.decode("UTF-8") == "print":
                ...
                fullNodePort = 10000  # other full node, used to send the blocks // CHANGE DEPENDING ON IF THIS IS F1 OR F2
                serverName = '127.0.0.1'
                fullnodeSocket = socket(AF_INET, SOCK_DGRAM)
                message = message.decode("utf-8")
